...
- `MulticloudProvider`/`MulticloudEnvironment` (`gymact.gyms.multicloud`): a **SIMULATED** multicloud IAM/storage/compute gym -- 20 capabilities across AWS/Azure/GCP (7/6/7), in-memory state only, no real cloud SDK calls and no credentials of any kind. Ontology (`ggen/multicloud-gym-pack/ontology.ttl`, untracked pending review) grounds every capability in `sosa:Procedure` (base typing) and `dct:title`/`dct:type` (SHACL-required), with `odrl:Permission` on the 3 policy-attach/role-assign operations, `org:Role` on `aws.iam.create_role`, `dcat:Distribution` on the 3 bucket/account-creation operations, and `prov:Entity`/`prov:Activity` on the 3 compute-provisioning operations -- zero new `rdfs:Class`/`owl:*Property` declared. `tests/test_multicloud.py`: 14 tests pass (`.venv/bin/python -m pytest tests/test_multicloud.py -v` -> `14 passed in 0.41s`), including a real rdflib ontology-vs-`CAPABILITY_REGISTRY` parity check; zero matches on the required mock-usage grep. `ggen/multicloud-gym-pack/` (`pack.toml` + templates, adapted from `ggen/consumer-bridge-pack-template/`) manufactures a Rust operation catalog, MCP tool schema, reference doc, and cross-check proof test from this ontology -- verified with a real `ggen sync run` (ggen 26.8.6): all 4 files written, real content, 20 operations correctly classified Read/Do.
