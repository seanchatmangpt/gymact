...
- `gymact.execution_loop` exactly-once across redelivery (PR #148 round 3): a run that actuated but ended without an admitted receipt (typed block, or refusal after actuation) is remembered by idempotency key; redelivering the same request typed-blocks with `TYPED_BLOCK_PRIOR_ACTUATION_UNRECONCILED` (`R_missing_consequence`) and zero new actuations, and a different request reusing that key is refused as `REFUSED_IDEMPOTENCY_KEY_CONFLICT`. Result ledger views and receipt evidence are deep copies (mutating one no longer rewrites the other). Zero-width/format-character identity fields and empty or blank `evidence_requirements` are refused at construction. New court `tests/explore_execution_loop/test_redelivery_exactly_once.py`.
