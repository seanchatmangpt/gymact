...
def e(g):
    return ProducerEpoch(Subject.parse("a/r@" + "a" * 40), g, f"{g:064x}", NOW)


class T(unittest.TestCase):
    def test_current_cut_qualifies_then_stales(self):
        ep = e(2)
        cur = {ep.subject.repo: ep}
        cut = EvidenceCut("current", 2, (ep,), NOW, NOW + timedelta(hours=1))
        obs = (Observation(ep, "REPOSITORY", Outcome.PASS, "ev", NOW),)
        q = qualify(
            subject=ep.subject.key,
            cuts=(cut,),
            current=cur,
            observations=obs,
            now=NOW,
            strategy=CutStrategy.MAX_FRESHNESS,
        )
