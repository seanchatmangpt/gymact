...
def test_new_failure_supersedes_stale_green():
    ...
    new = Evidence(subject, Epoch(now + timedelta(seconds=1)), "ci", "repository", Outcome.FAIL, "run-new")
    frontier = resolve_frontier((old, new), (Supersession(old, new, SupersessionReason.NEW_RUN),))
