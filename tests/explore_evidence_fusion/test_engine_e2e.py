...
def o(i, fam):
    return Observation(
        EvidenceSource("seanchatmangpt/gymact", f"r{i}", str(i) * 64, fam),
        "FOCUSED",
        "PASS",
        N,
        f"e{i}",
...
class T(unittest.TestCase):
    def test_independent_clusters_qualify_bounded_and_do_refuses(self):
        ...
        a, b = o(1, "fam1"), o(2, "fam2")
        pairs = {frozenset(("e1", "e2"))}
        q = qualify(
            subject=s,
            observations=[a, b],
        ...
        self.assertEqual(q.receipt.store, "SQLITE")
        with self.assertRaisesRegex(PermissionError, "UNRECEIPTED"):
            qualify(
                subject=s,
                observations=[a, b],
                graph=ProvenanceGraph(),
