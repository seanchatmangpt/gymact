...
class TestStrategies(unittest.TestCase):
    def test_three_strategies_remain_distinct(self):
        ...
        w = CompatibilityWitness(
            a.fingerprint, b.fingerprint, CompatibilityKind.BACKWARD_COMPATIBLE, "policy-proof"
        )
        self.assertEqual(decide(FreshnessStrategy.RESELECT, a, b).standing, "REQUALIFYING")
        self.assertEqual(
            decide(FreshnessStrategy.REBIND_EQUIVALENT, a, b, w).standing, "REQUALIFYING"
        )
        self.assertEqual(
            decide(FreshnessStrategy.REQUALIFY_COMPATIBLE, a, b, w).standing, "REQUALIFYING"
        )
        self.assertEqual(
            decide(FreshnessStrategy.REBIND_EQUIVALENT, a, b, None).standing, "UNKNOWN"
        )
