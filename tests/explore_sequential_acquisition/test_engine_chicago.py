...
class ChicagoCourt(unittest.TestCase):
    def test_constructs_bounded_non_actuating_plan(self):
        ...
        belief = BeliefState(("healthy", "fault"), (Fraction(1, 2), Fraction(1, 2)))
        policy = Policy("info", Strategy.MAX_INFORMATION, 3)
        scores = {sensor.digest: CandidateScore(sensor.digest, Fraction(3, 4), Fraction(1), Fraction(0), Fraction(1, 4), Fraction(1))}
        plan = construct_plan(subject="seanchatmangpt/gymact@" + "b" * 40, belief=belief, policy=policy, candidates=(sensor,), scores=scores, budget=Budget(Fraction(2), 20, 2), step=1)
