...
def test_current_binding_qualifies_without_do():
    ...
    e = Evidence(s, "b" * 64, "v1", "REPOSITORY", "PASS")
    c = ConsumptionClaim(s, s, "x", "b" * 64, "FOCUSED")
    out = qualify(
        c,
        e,
        EvidenceLease(now - timedelta(seconds=1), now + timedelta(seconds=1)),
        now,
        "b" * 64,
        "v1",
    )
    assert out["standing"] == "PARTIAL_ALIVE"
    assert not out["actuation_performed"]
