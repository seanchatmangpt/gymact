...
def test_mcp_tools_compile_to_authority_gated_structural_gym() -> None:
    spec = mcp_tools_to_gym_spec(
        subject_id="weather",
        endpoint_ref="stdio:weather",
        source_digest="mcp-digest",
        tools=[
            {
                "name": "get_weather",
                "title": "Weather",
...
def test_a2a_agent_card_skills_compile_without_promoting_claims_to_truth() -> None:
    spec = a2a_card_to_gym_spec(
        source_digest="card-digest",
        card={
            "name": "planner",
            "version": "1.0.0",
            "supportedInterfaces": [
            ...
            "skills": [{"id": "plan", "name": "Plan work", "description": "Produces a plan"}],
        },
    )
...
def test_lsp_server_capabilities_split_queries_from_edit_candidates() -> None:
    spec = lsp_initialize_to_gym_spec(
        subject_id="rust-analyzer",
        endpoint_ref="stdio:rust-analyzer",
        source_digest="lsp-digest",
        initialize_result={
            "serverInfo": {"name": "rust-analyzer", "version": "x"},
            "capabilities": {
                "hoverProvider": True,
