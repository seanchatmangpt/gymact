...
def test_conditional_idempotency_requires_fields() -> None:
    with pytest.raises(ValidationError, match="CONDITIONAL_IDEMPOTENCY_REQUIRES_KEY_FIELDS"):
        base_action(idempotency=IdempotencyClass.CONDITIONALLY_IDEMPOTENT)


def test_compensation_requires_reference() -> None:
    with pytest.raises(ValidationError, match="COMPENSATABLE_ACTION_REQUIRES_COMPENSATION_ACTION_REF"):
        base_action(reversal=ReversalClass.COMPENSATABLE)


def test_alive_requires_execution_evidence() -> None:
    with pytest.raises(ValidationError, match="ALIVE_ACTION_REQUIRES_EXECUTION_EVIDENCE"):
...
def test_quorum_means_multiple_oracles() -> None:
    with pytest.raises(ValidationError, match="QUORUM_VERIFICATION_REQUIRES_AT_LEAST_TWO_ORACLES"):
...
def test_uncertain_reconciliation_cannot_smuggle_retry() -> None:
    with pytest.raises(ValidationError, match="UNCERTAIN_OUTCOME_CANNOT_IMPLY_RETRY"):
        ReconciliationResult(
