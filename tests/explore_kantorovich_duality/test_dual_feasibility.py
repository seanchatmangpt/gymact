...
from gymact.explore_kantorovich_duality import DualityRefusal, DualPotential, GroundMetric


def test_dual_feasibility_and_reduced_costs() -> None:
    metric = GroundMetric.admit({"a", "b"}, {("a", "b"): 1, ("b", "a"): 1})
    potential = DualPotential(
        {"a": Fraction(0), "b": Fraction(0)}, {"a": Fraction(0), "b": Fraction(0)}
    )
...
def test_infeasible_dual_refuses() -> None:
    ...
    with pytest.raises(DualityRefusal, match="DUAL_INFEASIBLE"):
        DualPotential(
            {"a": Fraction(2), "b": Fraction(0)}, {"a": Fraction(0), "b": Fraction(0)}
        ).admit({"a", "b"}, {"a", "b"}, metric)
