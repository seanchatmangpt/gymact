...
class RelationTest(unittest.TestCase):
    def test_explicit_independence_precedes_heuristic_correlation(self):
        ...
        left = RailObservation(
            VerificationRail(subject, "a", "pytest", "runtime", "py", "x"), "1", Outcome.PASS, now
        )
        right = RailObservation(
            VerificationRail(subject, "b", "pytest", "runtime", "py", "y"), "2", Outcome.PASS, now
        )
