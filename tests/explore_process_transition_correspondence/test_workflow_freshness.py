...
from gymact.explore_process_transition_correspondence.workflow import (
    WorkflowConclusion,
    WorkflowEvidence,
    workflow_state,
)
...
class WorkflowFreshnessCourt(unittest.TestCase):
    def test_head_and_time_are_independent_admission_axes(self) -> None:
        ...
        self.assertEqual(
            workflow_state(WorkflowEvidence(current, WorkflowConclusion.PENDING), current),
            ObligationState.UNKNOWN,
        )
        now = datetime(2026, 8, 23, 8, 20, tzinfo=UTC)
