...
def test_connection_binds_real_manufacturing_bundle_without_actuation(tmp_path: Path)->None:
    ...
    assert out_a.read_bytes()==out_b.read_bytes(); assert first==second; assert first["stage"]=="EXERCISE"; assert first["authority"]=={"ceiling":"BOUNDED_GYM","do_authority":False}; assert first["parent"]["digest"]==sha256_bytes(parent_raw); assert first["standing"]["state"]=="PARTIAL_ALIVE"; roles=[item["role"] for item in first["artifacts"]]; assert roles==["gymact:manufacturing-bundle"]*4; assert {Path(item["path"]).name for item in first["artifacts"]}=={"profile.ttl","profile.shacl.ttl","runtime-contract.jcs.json","synthetic-ocel-result-contract.jcs.json"}
