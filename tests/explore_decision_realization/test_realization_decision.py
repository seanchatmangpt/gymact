...
def test_realized_regret_defer_and_selector_plurality() -> None:
    ...
    rates = directional_rates(
        (
            LabeledDecision(Decision.INDEPENDENT, False),
            LabeledDecision(Decision.DEPENDENT, True),
            LabeledDecision(Decision.DEFER, True),
            LabeledDecision(Decision.INDEPENDENT, True),
        )
    )
    ...
    selected = [select(candidates, strategy).name for strategy in Strategy]
    assert selected == ["risk", "safe", "info", "defer"]
    assert {item.name for item in frontier(candidates)} == {
        "risk",
        "safe",
        "info",
        "defer",
