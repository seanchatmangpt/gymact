...
def test_independence_aware_chicago_caps_positive_and_preserves_red_failure():
    ...
    calibration = Calibration.from_cases(
        3,
        "d",
        (
            ValidationCase("a", Fraction(0), Fraction(1), Fraction(1, 2)),
            ValidationCase("b", Fraction(0), Fraction(1), Fraction(1, 2)),
        ),
    )
    candidate = Candidate(
    ...
    bad = qualify(
        ...
        Strategy.MAX_COVERAGE,
        REQUIRED,
        (Standing.ALIVE, Standing.BUILD_BROKEN),
        ("e1",),
    )
