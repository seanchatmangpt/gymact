...
The SREGym provider remains the existing `gymact.gyms.sregym.SregymVendorProvider`. gdmcp is an **ADAPT/composition layer**, not another provider and not another MCP server. It compiles deterministic client-side calls to the already-real SREGym MCP surfaces.
