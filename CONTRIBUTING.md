...
## ggen ownership boundary: gym class/data, never episode scripts or unit tests

`ggen/` packs in this repo generate the gym **class itself** and adjacent
derived data -- e.g. `chatman_state_gym.py`, `cloud_topology_gym.py`,
`cloud_topology_validation.py`, `k8s_resource_gym.py`. They never generate
`scripts/run_*_episode.py` (episode-runner scripts) or `tests/gyms/*_unit.py`
(unit tests). Those two files stay hand-maintained per gym, alongside the
two-proof requirement above.

**Verified, not assumed:** every `ggen/*/templates/*.tmpl`'s `to:` target was
grepped across all packs (`grep -rn "^to:" ggen/*/templates/`). None targets
`scripts/run_*_episode.py` or `tests/gyms/*` -- the generated targets are gym
classes/data (`src/gymact/gyms/*.py`), registries, Rust operation catalogs,
WIT contracts, and docs, never episode-runner scripts or unit tests.

**Precedent:** commit `efc34c5` established this in writing. It corrected an
earlier, wrongly-located out-of-tree pack
(`~/ggen/packs/gymact-episode-pack/`) that had generated episode scripts and
unit tests for `chatman_state`/`cloud_topology` from *outside* this repo (a
staging/+cp workaround whose receipt described staging paths, not these real
files -- un-receipted drift risk, and it never composed with this repo's own
`ggen/` packs). After adversarial review, those four generated files
(`chatman_state_gym.py`, `cloud_topology_gym.py`,
`cloud_topology_validation.py`, `k8s_resource_gym.py`) were confirmed to have
always been generated by this repo's own `ggen/chatman-state-pack` and
`ggen/cloud-topology-validation-pack`, while the episode-runner/unit-test
layer for those same two gyms had only ever existed as output from the
deleted out-of-tree pack -- never from any pack under this repo's own
`ggen/`. The out-of-tree pack was deleted; the episode scripts and unit
tests were adopted as hand-maintained.
