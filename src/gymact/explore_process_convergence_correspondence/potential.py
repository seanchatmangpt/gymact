...
def weighted_l1(epoch: ClosureEpoch) -> Fraction:
    ...
    debt = sum(
        (o.weight * Fraction(int(o.state), int(State.FAIL)) for o in epoch.obligations), Fraction()
    )
    return debt / total
...
def lexicographic_vector(epoch: ClosureEpoch) -> tuple[int, ...]:
    return tuple(
        sum(1 for o in epoch.obligations if o.state == state) for state in reversed(tuple(State))
    )
