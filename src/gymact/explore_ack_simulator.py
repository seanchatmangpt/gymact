...
def simulate(event_id: str, consumers: tuple[Subject, ...], plan: FailurePlan) -> tuple[Witness, ...]:
    rng = random.Random(plan.seed)
    output: list[Witness] = []
    for consumer in sorted(consumers, key=lambda c: c.key):
        sequence = 1
        for kind in (WitnessKind.DELIVERED, WitnessKind.ACKNOWLEDGED, WitnessKind.DISCHARGED):
            admitted = False
            for _ in range(plan.max_retries + 1):
                if rng.random() >= plan.drop_probability:
                    digest = f"{event_id}:{consumer.sha}:{sequence}" if kind is WitnessKind.DISCHARGED else ""
