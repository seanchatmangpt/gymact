...
if TYPE_CHECKING:
    # Always import for static analysis so `dspy.Signature`/`dspy.Module`/
    # `dspy.Prediction` annotations below resolve to the real module's
    # types instead of the runtime-Optional fallback -- pyright type-checks
    # only the TYPE_CHECKING branch, so this keeps every `dspy.X` reference
    # (including inside nested `__init__`/`forward` methods, where a plain
    # module-level `if dspy is not None:` guard does not narrow across a
    # function boundary) fully typed without an `assert`/`# type: ignore`
    # at each of the ~20 use sites below.
    import dspy
else:
    try:
        import dspy
...
if dspy is not None:
    ...
    class GenerateHypothesisPortfolio(dspy.Signature):
        ...
        goal: Goal = dspy.InputField(
            desc="the desired state this diagnosis must explain a gap toward"
        )
    ...
    class CommitDiagnosis(dspy.Signature):
        ...
        goal: Goal = dspy.InputField(
            desc="the desired state the diagnosis must explain a gap toward"
        )
        facts: list[Fact] = dspy.InputField(desc="facts already admitted by the kernel")
        hypotheses: list[AdmittedHypothesis] = dspy.InputField(
