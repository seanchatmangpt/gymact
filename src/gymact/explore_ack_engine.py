...
def qualify(event: Invalidation, consumers: tuple[Subject, ...], witnesses: tuple[Witness, ...], critical: frozenset[str], authority: Authority) -> Qualification:
    ...
    keys = frozenset(c.key for c in consumers)
    results = tuple(evaluate(protocol, admitted.frontier, keys) for protocol in protocols)
    frontier = pareto(results)
    selected = sorted(frontier, key=lambda r: (not r.safe, not r.complete, r.protocol))[0]
