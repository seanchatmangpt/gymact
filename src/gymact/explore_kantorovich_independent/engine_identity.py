...
def admit_independent(verifier: EngineIdentity, manufacturer: EngineIdentity) -> None:
    if not verifier.independent_of(manufacturer):
        raise IndependentVerifierRefusal(
            "PSEUDO_INDEPENDENT_VERIFIER", f"{verifier.implementation}/{verifier.model}"
        )


INDEPENDENT_ENGINE = EngineIdentity(
    "kantorovich-duality/v1", "equation-verifier", "raw-primal-dual"
)
MANUFACTURER_ENGINE = EngineIdentity(
    "kantorovich-duality/v1", "certificate-manufacturer", "composed-certify"
)
