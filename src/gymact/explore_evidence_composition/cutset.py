...
def minimal_cut_sets(
    requirements: dict[str, frozenset[str]], failed: frozenset[str]
) -> tuple[frozenset[str], ...]:
    ...
    for size in range(1, len(universe) + 1):
        for combo in combinations(universe, size):
            ...
            if (
                groups
                and all(candidate & group for group in groups)
                and not any(existing < candidate for existing in cuts)
            ):
