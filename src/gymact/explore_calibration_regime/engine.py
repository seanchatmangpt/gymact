...
def qualify_regime(
    *,
    ...
    results = tuple(
        evaluate(s, drift=drift, cusum_alarm=cusum_alarm, support=8)
        for s in ("WINDOW_L1", "PREQUENTIAL_CUSUM", "MINIMAX_CURRENT")
    )
    frontier = pareto(results)
    standing = resolve(
        regime_state=regime.state, evidence_outcomes=evidence_outcomes, blockers=blockers
    )
    store = select(durable=durable, transactional=transactional)
    receipt = issue(
        {
            "subject": f"{subject.repo}@{subject.sha}",
            "generation": regime.generation,
