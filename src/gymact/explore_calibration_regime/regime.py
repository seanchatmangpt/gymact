...
class CalibrationRegime:
    ...
    def __post_init__(self):
        if self.generation < 0:
            raise Refusal("REFUSED_NEGATIVE_REGIME_GENERATION")
        if self.state not in {"STABLE", "DRIFT", "INSUFFICIENT"}:
            raise Refusal("REFUSED_INVALID_REGIME_STATE")
...
def advance(previous, *, model_digest, state):
    generation = (
        ...
        + (model_digest != previous.model_digest or state != previous.state)
    )
