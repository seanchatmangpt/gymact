...
def qualify(*, intents: tuple[SelectionIntent,...], before: SelectionContext,
            after: SelectionContext, strategy: FreshnessStrategy, now: datetime,
            witness: CompatibilityWitness|None=None, durable: bool=False,
            transactional: bool=False) -> Qualification:
    ...
    receipt=manufacture({"subject":before.subject.identity,"intent_nonce":frontier.current.nonce,
                         "before":before.fingerprint,"after":after.fingerprint,
                         "strategy":strategy.value,"standing":decision.standing,
                         "reusable":decision.reusable,"store":store.kind.value})
