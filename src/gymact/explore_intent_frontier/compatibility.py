...
def admit_witness(before: SelectionContext, after: SelectionContext, witness: CompatibilityWitness) -> None:
    if witness.before_fingerprint != before.fingerprint or witness.after_fingerprint != after.fingerprint:
    ...
    if witness.kind is CompatibilityKind.SEMANTIC_EQUIVALENT:
        if before.cut_digest != after.cut_digest or before.strategy != after.strategy or before.policy_digest != after.policy_digest:
            raise ValueError("REFUSED_UNPROVEN_SEMANTIC_EQUIVALENCE")
    if witness.kind is CompatibilityKind.BACKWARD_COMPATIBLE:
        if before.cut_digest != after.cut_digest or before.strategy != after.strategy:
