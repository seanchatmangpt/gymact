...
def classify(before: SelectionContext, after: SelectionContext) -> Drift:
    ...
    changed=[]
    if (before.cut_id,before.cut_digest,before.cut_generation)!=(after.cut_id,after.cut_digest,after.cut_generation):
        changed.append("cut")
    if before.strategy != after.strategy:
        changed.append("strategy")
    if before.policy_digest != after.policy_digest:
        changed.append("policy")
    if not changed: kind=DriftKind.UNCHANGED
    elif len(changed)>1: kind=DriftKind.MULTIPLE
    else: kind={"cut":DriftKind.CUT,"strategy":DriftKind.STRATEGY,"policy":DriftKind.POLICY}[changed[0]]
