...
def discover() -> tuple[StoreCandidate,...]:
    return (StoreCandidate(StoreKind.MEMORY,False,False),
...
def select(*, durable: bool, transactional: bool) -> StoreCandidate:
    viable=[c for c in discover() if (not durable or c.durable) and (not transactional or c.transactional)]
    if not viable: raise ValueError("REFUSED_NO_LAWFUL_STORE_CANDIDATE")
