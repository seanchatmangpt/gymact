...
def two_point_extremes(
    center: FiniteDistribution, radius: Fraction
) -> tuple[FiniteDistribution, ...]:
    if radius < 0 or radius > 1:
        raise refuse("INVALID_RADIUS", "two-point TV radius must be in [0,1]")
    if len(center.mass) != 2:
        raise refuse(
            "UNSUPPORTED_ADVERSARY_DIMENSION",
            "bounded adversary currently requires two-point support",
        )
