...
def wasserstein_1(
    ...
    if left.support != right.support:
        raise refuse(
            "UNSUPPORTED_WASSERSTEIN_SUPPORT",
            "bounded exact solver requires shared ordered support",
        )
    keys = sorted(left.support)
    if len(keys) != 2:
        raise refuse(
            "UNSUPPORTED_WASSERSTEIN_DIMENSION",
            "exact bounded solver currently admits two-point support",
        )
    a, b = keys
