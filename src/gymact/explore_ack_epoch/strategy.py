...
def complete(strategy: Strategy, states: dict[str, str]) -> bool:
    ...
    if strategy.name == "ALL":
        return bool(states) and len(done) == len(states)
    if strategy.name == "QUORUM":
        return bool(states) and len(done) >= (len(states) // 2 + 1)
    if strategy.name == "CRITICAL_PATH":
        return bool(strategy.critical) and set(strategy.critical) <= done
