...
def vector(
    inventory: ModuleInventory, policy: CollectorPolicy, package_dirs: set[str] | None = None
) -> StrategyVector:
    ...
    reversibility = {
        CollectorPolicy.IMPORTLIB: 4,
        CollectorPolicy.PACKAGE_NAMESPACE: 3,
        CollectorPolicy.UNIQUE_BASENAME: 2,
        CollectorPolicy.PATH_HASH_ALIAS: 4,
    }[policy]
...
def pareto(vectors: tuple[StrategyVector, ...]) -> tuple[StrategyVector, ...]:
    def dominates(a: StrategyVector, b: StrategyVector) -> bool:
        better_or_equal = (
            ...
            and a.reversibility >= b.reversibility
        )
        strict = (
            a.safety > b.safety
            or a.migration_cost < b.migration_cost
            or a.reversibility > b.reversibility
        )
