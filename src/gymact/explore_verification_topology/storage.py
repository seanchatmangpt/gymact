...
def select_store(
    require_durable: bool = False, require_transactional: bool = False
) -> StoreCandidate:
    for candidate in discover_stores():
        if (not require_durable or candidate.durable) and (
            not require_transactional or candidate.transactional
        ):
            return candidate
