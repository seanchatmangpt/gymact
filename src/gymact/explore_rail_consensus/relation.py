...
def relation(
    left: RailObservation, right: RailObservation, proofs: tuple[IndependenceProof, ...] = ()
) -> EvidenceRelation:
    ...
    for proof in proofs:
        if proof.matches(left.rail.fingerprint, right.rail.fingerprint):
            return (
                EvidenceRelation.INDEPENDENT if proof.independent else EvidenceRelation.CORRELATED
            )
