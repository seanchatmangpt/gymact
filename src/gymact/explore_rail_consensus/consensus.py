...
def evaluate(clusters: tuple[EvidenceCluster, ...], calibrations: dict[str, RailCalibration], strategy: ConsensusStrategy) -> ConsensusResult:
    fail_clusters = 0
    pass_clusters = 0
    calibrated_pass = 0
    for cluster in clusters:
        ...
        if outcomes == {Outcome.PASS.value}:
            pass_clusters += 1
            best = any(calibrations.get(obs.rail.fingerprint) and calibrations[obs.rail.fingerprint].state() == "CALIBRATED" for obs in cluster.members)
            calibrated_pass += int(best)
    if fail_clusters:
        standing = "BUILD_BROKEN"
    elif strategy is ConsensusStrategy.ALL_INDEPENDENT:
        standing = "PARTIAL_ALIVE" if clusters and pass_clusters == len(clusters) and len(clusters) >= 2 else "UNKNOWN"
