...
def correlated_clusters(
    observations: tuple[RailObservation, ...], proofs: tuple[IndependenceProof, ...] = ()
    ...
    for i, left in enumerate(observations):
        for j in range(i + 1, len(observations)):
            if relation(left, observations[j], proofs) in {
                EvidenceRelation.SAME_EVIDENCE,
                EvidenceRelation.CORRELATED,
            }:
                union(i, j)
    ...
    return tuple(
        EvidenceCluster(tuple(sorted(items, key=lambda x: x.evidence_id)))
        for _, items in sorted(groups.items(), key=lambda kv: min(o.evidence_id for o in kv[1]))
    )
