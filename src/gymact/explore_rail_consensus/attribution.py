...
class AttributionGraph:
    ...
    def components_for(self, signature: FailureSignature) -> tuple[str, ...]:
        return tuple(
            sorted(
                {edge.component for edge in self.edges if edge.signature_digest == signature.digest}
            )
        )
    ...
    def admit_closed(self) -> None:
        ...
        dangling = [
            edge.signature_digest for edge in self.edges if edge.signature_digest not in known
        ]
        if dangling:
