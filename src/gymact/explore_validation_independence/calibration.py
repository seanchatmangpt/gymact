...
class Calibration:
    ...
    @classmethod
    def from_cases(cls, generation: int, digest: str, cases: tuple[ValidationCase, ...]) -> "Calibration":
        if not cases:
        ...
        return cls(generation, len(cases), Fraction(covered, len(cases)), widths / len(cases), digest)
