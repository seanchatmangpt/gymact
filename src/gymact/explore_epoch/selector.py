...
def select_store(
    *, durable: bool, transactional: bool, candidates: tuple[StoreCandidate, ...] | None = None
) -> StoreCandidate:
    pool = candidates or discover()
    viable = [
        c
        for c in pool
        if (not durable or c.durable) and (not transactional or c.transactional) and c.reversible
    ]
    if not viable:
