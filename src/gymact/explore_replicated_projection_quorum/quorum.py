...
def assess_quorum(
    ...
    if len(observations) < universe.quorum_size:
        return QuorumAssessment(QuorumState.PARTIAL_VISIBILITY, "UNKNOWN", coverage, None, None, None, ())
    ...
    if len(highest_digests) > 1:
        return QuorumAssessment(QuorumState.SPLIT_BRAIN, "BLOCKED", coverage, highest, None, None, ())
    counts = Counter((item.generation, item.projection_digest) for item in observations)
    (generation, digest), count = max(counts.items(), key=lambda item: (item[1], item[0][0], item[0][1]))
    agreeing = tuple(sorted(
        item.replica_id for item in observations
    ...
    if generation < highest:
        return QuorumAssessment(QuorumState.STALE_MAJORITY, "UNKNOWN", coverage, highest, generation, digest, agreeing)
    return QuorumAssessment(QuorumState.HEALTHY, "PARTIAL_ALIVE", coverage, highest, generation, digest, agreeing)
