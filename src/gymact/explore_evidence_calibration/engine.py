...
class Qualification:
    ...
    selected_strategy: FusionStrategy
...
def qualify(
    ...
    *,
    ...
    results: tuple[FusionResult, ...] = tuple(
        evaluate(strategy, witnesses, estimate_map) for strategy in FusionStrategy
    )
    frontier = pareto(tuple(vector(result) for result in results))
    chosen = (
        next(r for r in results if r.strategy is FusionStrategy.MINIMAX_UNDER_SUPPORT)
        if any(r.under_calibrated for r in results)
        else next(r for r in results if r.strategy is FusionStrategy.CALIBRATED_LOG_ODDS)
    )
    decision = decide(chosen)
    if dep_blockers:
        decision = SequentialDecision(decision.decision, "BLOCKED", decision.score)
    store = select(durable=True, transactional=transactional)
    receipt = issue(
        {
            "subject": subject.exact_id,
            "strategy": chosen.strategy.value,
            "standing": decision.standing,
            "score": decision.score,
            "store": store.kind.value,
            "frontier": [v.strategy for v in frontier],
            "blockers": list(dep_blockers),
        }
    )
    return Qualification(
        ...
        chosen.strategy,
        store,
        tuple(vector.strategy for vector in frontier),
        dep_blockers,
        receipt,
    )
