...
def verify_correspondence(subject, a, b, metric, *, correlation: Fraction = Fraction(0), minimum: Fraction = Fraction(2)):
    ...
    standing = qualify(cost_gap=evidence.cost_gap, effective_evidence=n_eff, minimum=minimum, dependency_states=("ALIVE", "ALIVE"))
    receipt = Receipt(subject.identity, primal.engine, oracle.engine, str(evidence.cost_gap), standing.state)
