...
The complete commercial design space is powerless until one explicit selection
is supplied. Deterministic ggen agents manufacture the bounded internal actions,
GymAct executes those actions through its authority/receipt path, and the canonical
possibility graph proves that irreversible DO edges are never silently traversed.

No LLM, marketplace SDK, cloud credential, payment credential, seller account, or
external marketplace authority is used here.
"""
...
from gymact.commerce_dfcm_graph import commerce_dfcm_frontier
...
def commerce_agent_space() -> CombinationSpace:
    """Preserve logical organization independently of active execution WIP."""
...
def _pricing(plan: CommerceSelectedPlan) -> tuple[list[dict[str, Any]], str]:
    ...
    if plan.pricing_model == "subscription":
        return (
            [
                {
                    "dimension_id": "subscription_units",
                    "unit": "unit",
...
async def _execute_alive(
    *,
    ...
    if capability.consequence is Consequence.READ:
        effect = await runtime.read(
            episode_id,
            manufactured.capability,
            manufactured.payload,
        )
...
def _packaging(plan: CommerceSelectedPlan) -> dict[str, Any]:
    return {
        "helm_chart": True,
        ...
        "topology": plan.packaging_topology,
        "supply_chain_policy": plan.supply_chain_policy,
    }


async def execute_commerce_dfcm(plan: CommerceSelectedPlan) -> CommerceDfcmExecution:
    """Execute all bounded commerce semantics and stop before marketplace DO."""
    possibility = commerce_dfcm_frontier()
    ...
    async def alive_step(binding: str, payload: dict[str, Any]) -> None:
        nonlocal ordinal
        ordinal += 1
        item, receipt = await _execute_alive(
            agents=agents,
            runtime=runtime,
            episode_id=episode_id,
            binding=binding,
        ...
        alive.append(item)
        manufactured_receipts.append(receipt)
    ...
    async def refusal_step(
        ...
        nonlocal ordinal
        ordinal += 1
        item, receipt = await _execute_expected_refusal(
            agents=agents,
            runtime=runtime,
            episode_id=episode_id,
            binding=binding,
    ...
    await alive_step("agreement.admit", {"agreement": agreement})
    await alive_step("billing-authority.fence", {"agreement": agreement})
    ...
    create_event = _entitlement_event(
        plan,
        agreement_id=agreement_id,
        entitlement_id=entitlement_id,
        tenant_id=tenant_id,
    ...
    activate_event = {
        **create_event,
        "event_id": "entitlement-activate-primary",
        "kind": "ACTIVATE",
        "revision": 2,
    }
    entitlement_grant = _grant(entitlement_id, "entitlement.apply-event")
    await alive_step(
        "entitlement.apply-event",
        {"event": create_event, "grant": entitlement_grant},
    )
    await alive_step(
        "entitlement.lifecycle",
        {"event": activate_event, "grant": entitlement_grant},
    )

    if plan.entitlement_cardinality == "concurrent":
        second_agreement_id = f"{agreement_id}-2"
        second_entitlement = f"{entitlement_id}-2"
        second_agreement = {
            **agreement,
            "agreement_id": second_agreement_id,
            "offer_id": "offer-concurrent",
        }
        ...
        second_grant = _grant(second_entitlement, "entitlement.apply-event")
        await alive_step(
            "entitlement.apply-event",
            {"event": second_create, "grant": second_grant},
        )
        await alive_step(
            "entitlement.lifecycle",
            {"event": second_activate, "grant": second_grant},
        )
    ...
    await refusal_step(
        ...
        {
            "settlement_id": "settlement-without-provider",
            "acceptance_id": "fixture-acceptance",
        },
    ...
    for binding in (
        ...
        await alive_step(binding, {"packaging": packaging})

    await alive_step(
        "credit.construct",
        {
            "adjustment_id": "credit-sla",
            "agreement_id": agreement_id,
            "amount_micros": 1,
            "reason": "simulation-sla",
        },
    )
    await alive_step(
        "refund.construct",
        {
            "adjustment_id": "refund-correction",
            "agreement_id": agreement_id,
            "amount_micros": 1,
            "reason": "simulation-correction",
        },
    )
    await alive_step(
        "agreement.amend",
    ...
    await alive_step(
        "entitlement.lifecycle",
        {"event": cancel_event, "grant": entitlement_grant},
    )
    await alive_step(
        "agreement.cancel",
        {
            "agreement_id": agreement_id,
            "grant": _grant(agreement_id, "agreement.cancel"),
        },
    )
    ...
    if set(possibility.external_do_bindings) != set(external):
        raise RuntimeError("COMMERCE_DFCM_GRAPH_RUNTIME_EXTERNAL_FRONTIER_DRIFT")
    if not set(possibility.bounded_internal_do_bindings).issubset(exercised):
        raise RuntimeError("COMMERCE_DFCM_INTERNAL_DO_NOT_EXERCISED")
    ...
    return CommerceDfcmExecution(
        ...
        verified=True,
        episode_id=episode_id,
        possibility_graph_digest=possibility.graph_digest,
        ...
        reversible_capability_count=possibility.reversible_capability_count,
        do_frontier_count=possibility.do_frontier_count,
        bounded_internal_do_count=possibility.bounded_internal_do_count,
        ...
        exercised_refusal_bindings=tuple(sorted(set(refused))),
