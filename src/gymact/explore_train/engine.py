...
def execute(candidate: CandidateContract, runtime: RuntimeCandidate, payload: dict, rule: AdmissionRule) -> ExperimentOutcome:
    decision = admit(candidate, rule)
    if not decision.admitted: raise PermissionError(decision.reason)
    output = runtime.run(payload)
    receipt = Receipt(candidate.digest(), "CONSTRUCT", digest_value(payload), digest_value(output))
