...
class ConformanceResult:
    ...
    def standing(self) -> str:
        return "PARTIAL_ALIVE" if not self.failures and self.vector_count else "BUILD_BROKEN"
...
def _invoke(command: Sequence[str], request: dict) -> dict:
    ...
    if p.returncode != 0:
        raise RuntimeError(f"BLOCKED:IMPLEMENTATION_EXIT:{p.returncode}")
    lines = [x for x in p.stdout.splitlines() if x.strip()]
    if len(lines) != 1:
        raise ValueError("REFUSED:NON_SINGLE_RESPONSE")
    out = json.loads(lines[0])
    if not isinstance(out, dict):
        raise ValueError("REFUSED:NON_OBJECT_RESPONSE")
    return out
...
def conform(command: Sequence[str], vectors_path: Path) -> ConformanceResult:
    ...
    failures = []
    for v in vectors:
        actual = _invoke(command, v["request"])
        expected = v["expect"]
        if all(actual.get(k) == val for k, val in expected.items()):
            passed.append(v["id"])
        else:
