...
families execute against one deterministic semantic state machine; exact
request/response fixtures replay empirically observed behavior.

Structural execution is *not* exactness evidence. Every structurally inferred
response is labeled ``DISCOVERY_INFERRED``. Only an exact empirical replay is
labeled ``EMPIRICAL_REPLAY``. The whole-GCP crown lives in ``gcp_crown`` and
requires paired differential and case-level closure.

This module has no Google SDK, credential, or network dependency.
"""
...
def canonical_request_digest(method_id: str, payload: Mapping[str, Any]) -> str:
    ...
    return blake3(_canonical_json(normalized).encode()).hexdigest()
...
def _set_mask_path(target: dict[str, Any], path: str, source: Mapping[str, Any]) -> None:
    ...
    for part in parts[:-1]:
        ...
        if not isinstance(current, dict):
            ...
            cursor[part] = current
        cursor = current
    cursor[parts[-1]] = deepcopy(source_cursor)
...
class GcpExactSimulatorEnvironment:
    ...
    def __init__(
        ...
        *,
        ...
        self._methods = {method.identity: method for method in method_tuple}
        if len(self._methods) != len(method_tuple):
            raise ValueError("GCP_SIMULATOR_METHOD_GRAPH_DUPLICATED")

        empirical_by_id = {rule.method_id: rule for rule in empirical_rules}
        if len(empirical_by_id) != len(tuple(empirical_rules)):
        ...
        for fixture in tuple(replay_fixtures):
            if not fixture.valid:
        ...
        for name, resource in (initial_resources or {}).items():
            if not isinstance(resource, Mapping):
                raise TypeError(f"initial resource {name!r} must be an object")
            self._resources[str(name)] = deepcopy(dict(resource))
    ...
    async def actuate(self, capability: Capability, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        before = self._snapshot()
        response = self._execute_request(method, rule, payload)
    ...
    def _service_enabled(self, api: str) -> bool:
        if not self._enforce_service_enablement:
            return True
        return api in self._enabled_services or f"{api}.googleapis.com" in self._enabled_services
    ...
    def _execute_request(
        ...
        if not self._service_enabled(method.api):
        ...
        if rule.long_running:
            # Quota is charged exactly once here, at request admission. Completion
            # later applies the already-admitted pending effect directly.
            return self._start_operation(method, rule, payload)
        return self._apply_effect(method, rule, payload)
    ...
    def _read_many(self, method: DiscoveryMethod, payload: Mapping[str, Any]) -> GcpSimulatedResponse:
        ...
        body: dict[str, Any] = {"items": page, "_gymact_response_schema": method.response_schema}
    ...
    def _patch(self, payload: Mapping[str, Any]) -> GcpSimulatedResponse:
        ...
        if isinstance(raw_mask, str) and raw_mask.strip():
            ...
            for path in (item.strip() for item in raw_mask.split(",")):
                _set_mask_path(updated, path, body)
        else:
    ...
    def _iam_get(self, payload: Mapping[str, Any]) -> GcpSimulatedResponse:
        ...
        return _ok(self._iam.get(target, {"bindings": [], "etag": ""}))
    ...
    def _iam_set(self, payload: Mapping[str, Any]) -> GcpSimulatedResponse:
        ...
        if not target:
            return _google_error(400, "INVALID_ARGUMENT", "resource is required")
        body = _object(payload, "body")
        raw_policy = body.get("policy", body)
    ...
    def _iam_test(self, payload: Mapping[str, Any]) -> GcpSimulatedResponse:
        ...
        for binding in self._iam.get(target, {}).get("bindings", []):
            if isinstance(binding, Mapping) and isinstance(binding.get("permissions"), list):
                allowed.update(str(item) for item in binding["permissions"])
        return _ok({"permissions": [item for item in requested if item in allowed]})
    ...
    def _operation_name(self, payload: Mapping[str, Any]) -> str | None:
        name = _object(payload, "path_params").get("name")
    ...
    def _operation_list(self, payload: Mapping[str, Any]) -> GcpSimulatedResponse:
        ...
        operations = [
            self._public_operation(operation)
    ...
    def _public_operation(operation: Mapping[str, Any]) -> dict[str, Any]:
        return {key: deepcopy(value) for key, value in operation.items() if not key.startswith("_")}
    ...
    def _start_operation(
        ...
        operation = {
            "name": operation_name,
            "done": False,
            "metadata": {"method": method.identity, "submittedAtLogicalClock": self._clock},
    ...
    def _complete_due_operations(self) -> None:
        for operation in self._operations.values():
            ...
            rule = self._rules[method_id]
            # Deliberately call _apply_effect, not _execute_request: service
            # admission and quota were already decided at submission time.
            response = self._apply_effect(method, rule, pending["payload"])
            operation["done"] = True
...
class GcpExactSimulator:
    ...
    The method graph is configuration-derived, so consumers register this
    provider instance directly rather than flattening it into GymAct's static
    built-in registry.
    """
    ...
    async def materialize(
        ...
        *,
        ...
        if not all(isinstance(value, Mapping) for value in raw_methods):
            raise TypeError("config.methods entries must be objects")
        methods = tuple(_method_from_mapping(value) for value in raw_methods)
        ...
        raw_quota_limits = config.get("quota_limits", {})
        if not isinstance(raw_quota_limits, Mapping):
            raise TypeError("config.quota_limits must be an object")
        quota_limits: dict[str, int] = {}
        for key, value in raw_quota_limits.items():
