...
class Cell:
    ...
    def __post_init__(self) -> None:
        ...
        require(self.source_mass >= 0 and self.target_mass >= 0, "INVALID_MASS", "mass must be nonnegative")
...
def normalize(cells: tuple[Cell, ...]) -> tuple[Cell, ...]:
    ...
    require(source_total > 0 and target_total > 0, "ZERO_POPULATION", "source and target totals must be positive")
    return tuple(Cell(c.name, c.source_mass / source_total, c.target_mass / target_total) for c in cells)
