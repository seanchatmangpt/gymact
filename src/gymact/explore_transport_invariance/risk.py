...
def horvitz_thompson(losses: tuple[Fraction, ...], weights: tuple[Fraction, ...]) -> Fraction:
    require(len(losses) == len(weights) and bool(losses), "MISALIGNED_EVIDENCE", "losses and weights must align")
    return sum((loss * weight for loss, weight in zip(losses, weights, strict=True)), Fraction()) / len(losses)
...
def self_normalized(losses: tuple[Fraction, ...], weights: tuple[Fraction, ...]) -> Fraction:
    ...
    return sum((loss * weight for loss, weight in zip(losses, weights, strict=True)), Fraction()) / denominator
